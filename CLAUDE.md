...
- **`claude-review.yaml`** has three jobs keyed on `pull_request.user.login`: Dependabot PRs, `update-catalog` PRs (catalog diff — removals need confirming against the live spec), and everything else. Auth is OIDC federation via org-wide `ANTHROPIC_*` variables, not secrets — which is why it also works on Dependabot-triggered runs. Note the action refuses to run when the workflow file differs from the copy on `main`, so changes to it can't be reviewed by itself; they take effect after merge.
